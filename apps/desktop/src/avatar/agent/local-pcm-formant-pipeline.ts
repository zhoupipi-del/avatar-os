/**
 * Day13C — Local PCM Fixture Formant Pipeline
 *
 * 把 Day13C 的 PCM fixture provider、Day11C 的频谱探针、Day11B 的共振峰分析器
 * 串成一条端到端测试闭环：
 *
 *   provider.speak(text)
...
import { LocalPcmFixtureTtsProvider, LocalPcmFixturePlaybackSession } from "./local-pcm-fixture-tts-provider";
...
export interface LocalPcmPipelineResult {
  /** 本次会话的文本 */
...
export interface LocalPcmFormantPipelineOptions {
...
/**
 * PCM fixture provider -> formant 口型 的测试闭环管线。
 * 不写表情、不接 VRM、不依赖真实浏览器音频。
 */
export class LocalPcmFormantPipeline {
  private readonly provider: LocalPcmFixtureTtsProvider;
  private readonly defaultFrameCount: number;
  private readonly noiseGate: number | undefined;
  private session: LocalPcmFixturePlaybackSession | null = null;
  private probe: FormantVisemeRuntimeProbe | null = null;

  constructor(provider: LocalPcmFixtureTtsProvider, options?: LocalPcmFormantPipelineOptions) {
   ...
   */
  speakAndAnalyze(text: string, frameCount?: number): LocalPcmPipelineResult {
    const frames =
      Number.isFinite(frameCount) && (frameCount ?? 0) > 0
        ? Math.floor(frameCount!)
        : this.defaultFrameCount;
    let session: LocalPcmFixturePlaybackSession;
    try {
...
/** 工厂：用 PCM fixture provider 组装一条完整测试闭环管线。 */
export function createLocalPcmFormantPipeline(
  provider: LocalPcmFixtureTtsProvider,
  options?: LocalPcmFormantPipelineOptions,
): LocalPcmFormantPipeline {
  return new LocalPcmFormantPipeline(provider, options);
}
